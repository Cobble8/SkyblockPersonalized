...
public class DungeonsPartyThread extends Thread {
	public static ArrayList<String> partyMemberList = new ArrayList();
	...
	public void run() {
		...
		ChatRecieveEvent.currentPartyUsing="dungeons";
		...
		for(int i=0; i<partyMemberList.size();i++) {
			apiEnabled.add(true);
			String currMember = partyMemberList.get(i);//tempArray[0];
			...
			} catch (Exception e1) {apiEnabled.set(i, false); return;}
			String uuid = GetFromAPI.getUUID(getuuid);
			String newInfo = "";
			
			
			try {
				newInfo = HttpClient.readPage("https://api.hypixel.net/skyblock/profiles?key="+(String) DataGetter.find("APIKey")+"&uuid="+uuid);
			} catch (Exception e1) {apiEnabled.set(i, false); return;}
			String achievementsInfo = "";
			try {
				achievementsInfo = HttpClient.readPage("https://api.hypixel.net/player?key="+(String) DataGetter.find("APIKey")+"&uuid="+uuid);
			} catch (Exception e1) {apiEnabled.set(i, false); return;}
			
			
			nameList.add(currMember);
			String profileuuid = GetFromAPI.getCurrProfile(newInfo, GetFromAPI.getUUID(getuuid));
			//Dungeons.checkCataLevel(GetFromAPI.getClassXP(newInfo, uuid, GetFromAPI.getCurrProfile(newInfo, GetFromAPI.getUUID(getuuid));
			//int classLevel = Dungeons.checkCataLevel(GetFromAPI.getClassXP(newInfo, GetFromAPI.getUUID(getuuid), profileuuid));
		...
		for(int i=0; i<nameList.size();i++) {
			...
			if(apiEnabled.get(i)) {
				...
				Utils.sendErrMsg(Colors.AQUA+nameList.get(i)+Colors.YELLOW+":"+Colors.RED+" API Disabled");
			}
