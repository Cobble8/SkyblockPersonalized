...
public class Utils {
	...
	public static void drawString(Object[] texts, int x, int y, int textStyle, Boolean align) {
		...
		for(int i=0;i<text.size();i++) {
			...
			Utils.drawString(text.get(i), x+finXOff-(aO2), y+(10*i), finTextStyle);
	...
	public static void drawBoldScaledString(String string, int x, int y, double scale, int textStyle) {
		...
		String cleanString = Colors.BLACK+Utils.unformatText(string);
		...
		GlStateManager.scale(scale, scale, scale);
		Utils.drawString(cleanString, (int) (posX/scale)-1, (int) (posY/scale), textStyle);
		Utils.drawString(cleanString, (int) (posX/scale)+1, (int) (posY/scale), textStyle);
		Utils.drawString(cleanString, (int) (posX/scale), (int) (posY/scale)-1, textStyle);
		Utils.drawString(cleanString, (int) (posX/scale), (int) (posY/scale)+1, textStyle);
		Utils.drawString(Colors.WHITE+Utils.unformatText(string), (int) (posX/scale), (int) (posY/scale), textStyle);
		GlStateManager.popMatrix();
	}
		...
		if(DataGetter.findBool("core.skyblock.only")) {
			...
			if(title.contains("skyblock")) {
				...
				for(Score line : scoreboardLines) {
					...
					if(loc) {SBP.subLocation = strippedLine.toLowerCase(Locale.ENGLISH); break;}
